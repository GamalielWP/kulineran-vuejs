<template>
    ...
    <div class="container mt-5">
        <div v-for="pesanan in pesanansByTable" :key="pesanan.id" class="mb-4">
            <CardMenu
            :pesanan="pesanan"
            @delete-menu="deleteMenu"
            @cook-menu="cookMenu"></CardMenu>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        setPesanan(data) {
            this.pesanans = data;
            this.pesanansByTable = data.filter(d => d.station === 'table');
        },
        setPesananById(data) {
        ...
        updatedMenuStation(id) {
            this.pesananById.station = "kitchen";
            axios
