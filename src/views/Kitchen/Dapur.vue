<template>
    ...
    <div class="container mt-5">
        <div v-for="pesanan in pesanansByDapur" :key="pesanan.id" class="mb-4">
            <CardMenu
            :pesanan="pesanan"
            @delete-menu="deleteMenu"
            @serve-menu="serveMenu"></CardMenu>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        setPesanan(data) {
            this.pesanans = data;
            this.pesanansByDapur = data.filter(d => d.station === 'kitchen');
        },
        setPesananById(data) {
        ...
        updatedMenuStation(id) {
            this.pesananById.station = "served";
            axios
